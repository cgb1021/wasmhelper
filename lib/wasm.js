...
function WASM(instance) {
  ...
  this.byteLength = 0;
  var callbacks = [];
  var error = importObject && typeof importObject.error === 'function' ? importObject.error : null;
  var isInit = false;
  var functionsInTableMap = null;
  ...
  var init = function init(exports) {
    ...
    _this.table = exports.__indirect_function_table;
  ...
  this.setHeap = function () {
    var buf = _this.memory.buffer;
    _this.byteLength = buf.byteLength;
  ...
  this.ready = function (fn) {
    if (typeof fn === 'function') {
      if (isInit) fn.call(_this);else callbacks.push(fn);
    }

    return isInit;
  };

  this.fn2wasm = function (func) {
    var sig = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
    ...
    try {
      ...
      var msg = 'Unable to grow wasm table. Set ALLOW_TABLE_GROWTH.';
    ...
    var ret = _this.table.length - 1;

    try {
      _this.table.set(ret, func);
    } catch (err) {
      if (!(err instanceof TypeError)) {
        throw err;
      }

      var wrapped = _utils["default"].convertJsFunctionToWasm(func, sig);

      _this.table.set(ret, wrapped);
    }
  ...
  try {
    if (instance instanceof WebAssembly.Instance) {
      ...
      if (_typeof(importObject) !== 'object' || !importObject) {
        importObject = {};
      }
